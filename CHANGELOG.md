...
* Added an `android-build` CI job (`flutter build apk --debug` against the example app) alongside the existing `ios-build` job, since there was previously no automated Android compile check at all — this and the iOS build are the only automated verification for native code; Android was additionally exercised on a physical Pixel 9a (clean launch, ARCore session creation, and the `initialize()` config round-trip all verified with no exceptions), but a full interactive pass (tap-to-place, pan, rotate, anchor upload/download) still needs a manual device session. iOS remains compile-checked only — no iPhone was available to run it, consistent with holding off on a pub.dev publish until that happens.
