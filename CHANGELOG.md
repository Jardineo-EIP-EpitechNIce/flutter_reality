...
* Fixed a native memory leak on Android: removing an anchor (`ARAnchorManager.removeAnchor`) never destroyed the underlying `AnchorNode`'s Filament engine entity or cleared it from the plugin's internal map, so every anchor ever created stayed allocated for the life of the app. A second, likely larger leak from repeated model loading is still open — see `docs/device-testing-log.md`.
