# Changelog
## 0.1.1
* Bumped the Android `io.github.sceneview:arsceneview` dependency from `2.2.1` to `2.3.3` to fix the Android 15+ "not 16 KB page-size compatible" warning: `2.2.1` pulled in a Filament `1.52.0` build whose native libraries (`libfilament-jni`, `libfilament-utils-jni`, `libgltfio-jni`) had 4 KB-aligned `LOAD` segments, while `2.3.3` ships Filament `1.68.2`, aligned to 16 KB. No API or `compileSdk` change needed this time (unlike an earlier, reverted attempt to bump to `2.3.0` — see `docs/device-testing-log.md`, which didn't fix the alignment and forced `compileSdk` 34→35). Verified on a physical Pixel 9a.
