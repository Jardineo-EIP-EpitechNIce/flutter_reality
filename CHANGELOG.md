# Changelog
## Unreleased
* Bumped `geolocator` `^12.0.0` → `^14.0.3` and `permission_handler` `^11.3.1` → `^12.0.3` (a later `13.x` exists but its `permission_handler_android` requires `compileSdk 37`, one version past what the current AGP `9.0.1` supports — `flutter build apk` fails against it — so `12.0.3` is the newest that still compiles clean), and `json_annotation` `^4.9.0` → `^4.12.0`. No Dart API changes needed for any of the three (`flutter analyze`/`flutter test` unaffected); verified the example app still builds (`flutter build apk --debug`) against the new native Android dependency versions.
