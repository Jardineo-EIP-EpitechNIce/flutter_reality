...
class ARNode {
  /// Creates a node of the given [type], rendering the model at [uri].
  ///
  /// The initial transform can be given directly via [transformation], or
  /// composed from [position], [scale], [rotation] (an axis-angle
  /// `Vector4`), and/or [eulerAngles]; any combination left unset falls back
  /// to identity/no offset. [name] defaults to an autogenerated unique
  /// value, and [data] carries arbitrary caller-defined metadata that is
  /// never sent to the native platform.
  ARNode({
    required this.type,
    required this.uri,
    String? name,
  ...
  /// Write-only setter that sets the receiver's rotation from a quaternion
  /// directly, without going through [rotation]'s `Matrix3` representation.
  ...
  /// Holds any data attached to the node, especially useful when uploading
  /// serialized nodes to the cloud. It is passed through to the native
  /// Android/iOS side alongside the rest of the node (neither platform
  /// currently reads it, but don't rely on it staying purely on the Dart
  /// side — avoid putting anything here you wouldn't want crossing the
  /// platform channel).
...
/// Convenience accessors for reading and writing a [Matrix4]'s scale and
/// euler-angle components directly, decomposing/recomposing the matrix under
/// the hood.
extension Matrix4Extenstion on Matrix4 {
  /// The scale component of this matrix, extracted via decomposition.
  ...
  /// The rotation component of this matrix as euler angles (pitch, yaw,
  /// roll, in radians), extracted via decomposition.
  ...
  /// Sets this matrix's rotation from euler angles (pitch, yaw, roll, in
  /// radians), preserving its current translation and scale.
...
/// Converts a `ValueNotifier<Matrix4>` (as used by [ARNode.transformNotifier])
/// to and from the flat 16-value list representation used on the wire.
class MatrixValueNotifierConverter
  ...
  /// Rebuilds a `ValueNotifier<Matrix4>` from a flat list of 16 doubles.
  @override
  ValueNotifier<Matrix4> fromJson(List<dynamic> json) {
    return ValueNotifier(Matrix4.fromList(json.cast<double>()));
  }

  /// Flattens the current value of [matrix] into a list of 16 doubles.
  @override
